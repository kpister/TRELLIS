...
class SLatVaeGaussianTrainer(BasicTrainer):
    ...
    def _init_renderer(self):
        ...
        self.renderer.pipe.kernel_size = self.models["decoder"].rep_config[
            "2d_filter_kernel_size"
        ]

    def _render_batch(
        self, reps: List[Gaussian], extrinsics: torch.Tensor, intrinsics: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        for i, representation in enumerate(reps):
            render_pack = self.renderer.render(
                representation, extrinsics[i], intrinsics[i]
            )
            if ret is None:
    ...
    def _get_status(self, z: SparseTensor, reps: List[Gaussian]) -> Dict:
        ...
        xyz_base = (z.coords[:, 1:].float() + 0.5) / self.models[
            "decoder"
        ].resolution - 0.5
    ...
    def run_snapshot(
        ...
        for i in range(0, num_samples, batch_size):
            ...
            z = self.models["encoder"](
                args["feats"], sample_posterior=True, return_raw=False
            )
        ...
        ret_dict.update(
            {f"rec_image": {"value": render_results["color"], "type": "image"}}
        )
        ...
        ret_dict.update(
            {f"miltiview_image": {"value": miltiview_images, "type": "image"}}
        )
